...
for (j in 1:length(nset)){
  ...
  output45<-foreach(m = seq(1,size,by=1),.combine = 'rbind') %dopar% {
    ...
    X <- cbind(X1,X2)
    shubert_function <- function(x) {
      sum(1:5 * cos((1:5 + 1) * x[1]) * cos((1:5 + 1) * x[2]))
    }
    y <- 1.5*apply(X,1,shubert_function)*ID+   rnorm(n,1)
    C <-  rnorm(n,0,4)- rnorm(n,-1.5,1)+ 0.5*rnorm(n,1,1) # censoring variable
    #C <-  rnorm(n,0,4)- rnorm(n,-1.1,1)+ 0.5*rnorm(n,1,1) # censoring variable ID <-0
    delta <-(y<=C)
  ...
  output65<-foreach(m = seq(1,size,by=1),.combine = 'rbind') %dopar% {
    ...
    y <- 1.5*apply(X,1,shubert_function)*ID +  rnorm(n,1)
    C <-  rnorm(n,0,4)- rnorm(n,2.5,1)+ 0.5*rnorm(n,1,1) # censoring variable=0.65
    #C <-  rnorm(n,0,4)- rnorm(n,1.2,1)+ 0.5*rnorm(n,1,1) # censoring variable ID<-0
    delta <-(y<=C)
    Cr <- 1-mean(delta) # censoring rates
    Y<- pmin(y,C) # observed variable
