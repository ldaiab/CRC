...
for (j in 1:length(nset)){
  ...
  output45<-foreach(m = seq(1,size,by=1),.combine = 'rbind') %dopar% {
    # modulus function 
    X <- runif(n,-2,2)
    y <- ID*(5*(X - floor(X)) - 1) + rnorm(n,0,1)
    C <-  rnorm(n,2,0.5)-  rnorm(n,0.2,0.1)# censoring variable
    #C <-  rnorm(n,2,0.5)-  rnorm(n,1.9,0.1)# censoring variable for ID <-0
    delta <-(y<=C)
  ...
  output65<-foreach(m = seq(1,size,by=1),.combine = 'rbind') %dopar% {
    ...
    X <- runif(n,-2,2)
    y <- ID*(5*(X - floor(X)) - 1)+ rnorm(n,0,1)
    #C <-  rnorm(n,2,0.5)-  rnorm(n,1.3,0.1)# censoring variable
    C <-  rnorm(n,2,0.5)-  rnorm(n,2.45,0.1)# censoring variable for ID <-0
    delta <-(y<=C)
    Cr <- 1-mean(delta) # censoring rate
    Y<- pmin(y,C) 
